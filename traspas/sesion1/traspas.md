...
lo generará automáticamente el compilador si  nuestra clase está compuesta por tipos `Encodable`s. (`String`, `Int`, `Float`, `Date`, `Array`...)

- El protocolo `Decodable` declara un único inicializador:

```swift
init(from decoder: Decoder) throws
```

si nuestros campos son `Decodable`, nuestra clase/struct lo será automáticamente
...
- Podemos serializar/deserializar con nombres de campos distintos a los de la  estructura de datos

- `CodingKeys`:  enumeración de `String`s conforme al protocolo `CodingKey` con la asociación entre campos en el formato serializado y en nuestro código 
...
- Las preferenciasno son más que una *property list* en la que el objeto “raíz” es un `Dictionary`. El sistema se encarga de serializar/deserializar los valores a/desde un archivo `.plist` de modo automático.
